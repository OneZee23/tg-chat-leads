 ...
 *
 * Все четыре измерены на корпусе из 955 разосланных объявлений: суммарно
...
const STOP_PATTERNS: Array<[string, RegExp]> = [
  ...
  [
    'stop:вакансия',
    /ищ[уе]м?\s+(?:\S+\s+){0,2}(?:репетитор|преподавател|учител|педагог|тренер|наставник)/,
  ],
  ...
  // Продают репетиторам, а не преподают. Сюда попадают прямые конкуренты,
  // рекламирующиеся в тех же чатах, и инфобиз «где искать учеников».
  [
    'stop:продажа-репетиторам',
    /для\s+репетиторов|(?:бот|приложение|планировщик|crm|сервис|платформа|курс)\s+для\s+репетитор|где\s+искать\s+учеников/,
  ],
