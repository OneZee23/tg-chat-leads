...
describe('classifyReply', () => {
  ...
  it('похвала + «не нужно» без вопроса — на ручной разбор, не отказ', () => {
    ...
    expect(
      classifyReply(
        'платформа супер, всё удобно и лаконично, не нужно сильно разбираться',
      ),
    ).toBe('neutral');
  });
  ...
  it('тёплый лид С ВОПРОСОМ — это вопрос, а не отказ (ревью нашло на живых)', () => {
    ...
    expect(
      classifyReply(
        'это актуально? сейчас ищу такую платформу, попробую, но если уже не актуально',
      ),
    ).toBe('question');
    // «не нужно разбираться» — это похвала, а не отказ; плюс вопрос.
    expect(
      classifyReply(
        'платформа супер, всё удобно, не нужно разбираться, но хотел спросить...?',
      ),
    ).toBe('question');
  });
...
describe('autoReplyDecision', () => {
  ...
  it('фидбек про фичу (вкладку домашки) → тебе', () => {
    const d = autoReplyDecision(
      'я бы сделала вкладку за домашнюю работу, часто забываю задания',
    );
    expect(d.action).toBe('manual');
  });
});
