...
function renderEntry(e: ReviewEntry): string {
  ...
  const lines = [
    ...
    '',
    `- имя: ${e.displayName}`,
    // Роль заполняет человек. Автоматически её взять неоткуда: наш хук —
    // это ДОГАДКА о предмете по объявлению, и он ошибается. Живой пример:
    // хук говорил «по английскому», а человек преподаёт корейский.
    `- роль: `,
...
export function formatReviews(dump: ReviewsDump): string {
  ...
  const out: string[] = [
    ...
    '',
    'Заполни `- роль:` — что за преподаватель («Репетитор по биологии»).',
    'Бери из его собственного поста о себе, а НЕ из нашего хука: хук это наша',
    'догадка о предмете, и он ошибается. Пусто — на карточке будет просто',
    '«Преподаватель».',
