...
export function formatReviews(dump: ReviewsDump): string {
  ...
  const out: string[] = [
    ...
    '',
    'Под каждой записью директива. Оставь `PUBLISH` — поедет на лендинг.',
    'Замени на `SKIP` — не поедет. Лишние цитаты у записи удали, останется',
    'первая. Дальше: `yarn reviews:build <имя файла>`.',
    '',
    'Публикация НЕ пройдёт без явного разрешения — это персональные данные,',
    'и оператор по документам ты лично. Записи без разрешения собраны ниже',
    'отдельно: там сначала надо спросить.',
    '',
  ];
