...
describe('роль на карточке', () => {
  it('читает заполненную роль', () => {
    expect(parseReviews(FILE)[0].role).toBe('Преподаватель корейского');
  });

  it('пустая роль остаётся пустой, а не ломает разбор', () => {
    expect(parseReviews(FILE)[1].role).toBe('');
  });
